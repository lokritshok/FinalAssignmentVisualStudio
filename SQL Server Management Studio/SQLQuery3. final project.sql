...
CREATE PROCEDURE createLIBMANSYSTEMdb
AS
BEGIN
		   ...
		   CREATE TABLE LIBRARIAN
			...
			DROP TABLE LIBRARIAN,LIBRARY_BRANCH,BOOKS,BORROWER,PUBLISHER,BOOK_AUTHORS,BOOK_COPIES, BOOK_LOANS;
			...
			SELECT l1.Librarian_Name, b1.Title, b2.BranchName, c1.Number_Of_Copies
            FROM LIBRARIAN l1
			INNER JOIN  BOOKS b1 ON b1.BookID = l1.Librarian_BOOKS
			INNER JOIN  BOOK_COPIES c1 ON c1.BookCopiesID = l1.Librarian_BOOK_COPIES
			INNER JOIN  LIBRARY_BRANCH b2 ON b2.BranchID = l1.Librarian_LIBRARY_BRANCH
			INNER JOIN  BORROWER b3 ON b3.CardNo = l1.Librarian_BORROWER
			INNER JOIN  BOOK_LOANS b4 ON b4.BookID = l1.Librarian_BOOK_LOANS
			INNER JOIN  PUBLISHER p1 ON p1.PublisherName = l1.Librarian_PUBLISHER
			INNER JOIN  BOOK_AUTHORS b5 ON b5.BookAuthorID = l1.Librarian_BOOK_AUTHORS 
			WHERE Librarian_Name = 'V SHOK';
